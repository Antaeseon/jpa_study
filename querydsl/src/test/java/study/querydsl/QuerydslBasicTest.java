...
public class QuerydslBasicTest {
	...
	//getter setter가 없어도 동작, 필드에 바로 값을 넣어버린다.
	@Test
	...
	public void findUserDto() throws Exception {
		...
		//fields 매핑을 할 때에는 as를 해 줘야 매핑이 된다.
		List<UserDto> result = queryFactory
			.select(Projections.fields(UserDto.class,
				member.username.as("name"),
				// ExpressionUtils.as(member.username,"name") // 이 방식을 사용해도 된다!! 하지만 as가 더 낫다
				//sub쿼리는 어쩔 수 없이 ExpressionUtils를 사용해야 된다.
	...
	public void findUserDtoByConstructor() throws Exception {
		...
		List<UserDto> result = queryFactory
			.select(Projections.constructor(UserDto.class,
				member.username, member.age))
			// member.username, member.age,member.id)) 이런식으로 적어도 컴파일은 되어서 문제를 컴파일타임에 잡는 것이 불가능하다.
			.from(member)
			.fetch();

		for (UserDto member : result) {
			System.out.println("member = " + member);
		}
	}
	...
	//Predicate를 사용해도 되는데 BooleanExpression을 사용해야 조립이 된다.
	...
	public void bulkUpdate() throws Exception {
		...
		//영속성 컨텍스트가 우선이기 때문에 db값이랑 차이 발생
